...
Citizen.CreateThread(function()
	...
	while true do 
		...
		elseif not nui then
			idle = 5
			DrawMarker(36, zx[1],zx[2],zx[3]-0.27 ,0,0,0,0,0,1.0,1.0,1.0,1.0,255, 102, 0,200,0,0,0,1)
			local playercoords = GetEntityCoords(PlayerPedId())
			local distance = #(playercoords - zx)
			if distance < 2 then
...
AddEventHandler("Notify", function(a,b)
	ESX.ShowAdvancedNotification('Mechanic Menu', a, b, 'CHAR_CARSITE', 1)
end)
